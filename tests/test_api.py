...
def test_sp_matzip_topn_sorted(rng, dtype):
    ...
    # indices are not necessarily equal. For example in case of multiple equal scores to the topn boundary,
    # sp_matmul_topn and the zipped approach may pick different matches. In such cases for sp_matmul_topn
    # the insertion order in the maxheap is important, which is impossible to replicate in sp_matzip_topn_sorted,
